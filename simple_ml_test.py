...
diabetes_ml_props = {"from": "DIAGNOSES_ICD", "to":"DIAGNOSES_ICD", \
            "db_from": "subject_id, hadm_id, icd9_code", "c": .483293023857, "l1":.2, "flag":False,\
            "db_to": "subject_id, hadm_id, icd9_code", "results":["D_ICD_DIAGNOSES", "ICD9_CODE",\
            "LONG_TITLE"],"from_index": 2, "print_index": 0}
...
X, y = grouping_base.array_generation_for_ml_patient(diabetes_ml_props['to'],\
                                                     patient_data,\
...
query_string = ("SELECT {3} FROM mimiciii.{0} WHERE {1} in {2} LIMIT 10;").format\
